...
server '167.71.5.84', port: 22, roles: [:web, :app, :db], primary: true
...
namespace :deploy do
  ...
  task :check_revision do
    on roles(:app) do
      unless `git rev-parse HEAD` == `git rev-parse origin/ba-m`
        puts "WARNING: HEAD is not the same as origin/ba-m"
